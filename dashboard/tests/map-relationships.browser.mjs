...
try {
  for (const width of [1440, 390]) {
    ...
    // Two clicks in one task land before React re-renders the disabled button.
    await composer.getByRole('button', { name: 'Create handoff' }).evaluate(button => { button.click(); button.click() })
    ...
    await edgeControls.getByRole('button', { name: 'Remove handoff' }).click()
    await edgeControls.getByRole('button', { name: 'Confirm remove' }).evaluate(button => { button.click(); button.click() })
    await edgeControls.waitFor({ state: 'detached' })
    ...
    assert.equal(await summary.innerText(), 'Now: temperature 0.62 · max tokens 1600 · loop off')
    ...
    assert.equal(await page.evaluate(() => document.documentElement.scrollWidth > innerWidth), false)
    // Clearing the filter closes the panel, and picking the suite again does not reopen it.
    await legend.getByRole('button', { name: 'All agents' }).click()
    await suitePanel.waitFor({ state: 'detached' })
    await page.waitForFunction(() => document.querySelectorAll('[data-graph-node]').length === 7)
    await legend.getByRole('button', { name: 'research suite, 2 agents' }).click()
    await page.waitForFunction(() => document.querySelectorAll('[data-graph-node]').length === 4)
    assert.equal(await suitePanel.count(), 0)
    await legend.getByRole('button', { name: 'research suite, 2 agents' }).click()
    await page.waitForFunction(() => document.querySelectorAll('[data-graph-node]').length === 7)
