...
def _is_actionable(item: str) -> bool:
    """Filter worklog/changelog noise so only real backlog items become cards."""
    if len(item) < 8:
        return False
    if re.match(r"^\d{4}-\d{2}-\d{2}", item):  # dated worklog entry, not a task
        return False
...
def _parse_backlog_items(text: str, headings: list[str], max_items: int = 40) -> list[str]:
    """Pull actionable bullet items that live UNDER a capture heading.

    The git-discipline gate: only content beneath an explicit heading (## Backlog,
    ## Next, ...) becomes a card, and never under a "done/shipped" heading. Checked
    items (- [x]) and worklog/changelog noise are skipped. Deeply-nested bullets
    (detail lines) are skipped — only top-level items become cards.
    """
    ...
    for raw in text.splitlines():
        ...
        if not bm:
            continue
        if len(bm.group(1)) >= 4:  # nested detail line, not a top-level item
            continue
...
def _prerank(stories: list[dict]) -> list[dict]:
    """Cheap first pass so the LLM scores the *likely* top, not insertion order."""
    return sorted(
        stories,
        key=lambda s: (
            _PRIORITY_ORDER.get(s.get("priority", "P2"), 2),
...
def _render_backlog_doc(project_name: str, stories: list[dict]) -> str:
    """Canonical BACKLOG.md rendered FROM the store (dashboard = source of truth)."""
    rel = [
        s
        for s in stories
        if s.get("product") == project_name and s.get("status") in ("backlog", "active")
    ]
    rel.sort(
        key=lambda s: (
            _PRIORITY_ORDER.get(s.get("priority", "P2"), 2),
    ...
    out = [
        f"# {project_name} — Backlog",
        "",
        "> **Source of truth: the AIIA dashboard.** This file is generated by",
        "> `project_pulse`. Add or reprioritize items in the dashboard — edits here",
        "> are read-only context and will be overwritten on the next sync.",
        "",
    ]
    current = None
    for s in rel:
...
class TaskRunner:
    ...
    async def _task_project_pulse(self) -> str:
        ...
        # Rank the backlog. Cheap prerank first so the LLM scores the likely top of
        # the WHOLE backlog (not insertion order), then persist scores + suggested
        # priority back so the ranking is durable and drives the doc + cards.
        await self._progress("project_pulse", 78, "Prioritizing backlog")
        top: list[str] = []
        if self.story_prioritizer:
            try:
                candidates = _prerank(self.roadmap_store.list(status="backlog"))[:15]
                ranked = await self.story_prioritizer.prioritize_backlog(candidates, limit=15)
                for r in ranked:
                    self.roadmap_store.update(
                        r["id"],
                        priority=r.get("suggested_priority", r.get("priority", "P2")),
                        priority_score=r.get("priority_score", 0),
                        priority_reasoning=r.get("priority_reasoning", ""),
                    )
                top = [
                    f"{r.get('suggested_priority', r.get('priority', '?'))} "
                    f"{r.get('title', '')[:48]} [{r.get('product', '')}]"
                    for r in ranked[:3]
                ]
            except Exception as e:
                logger.warning("project_pulse prioritize failed: %s", e)

        # Source of truth: optionally regenerate each project's backlog doc FROM the
        # store. Opt-in per project (manage_backlog: true) so nothing is clobbered
        # until you approve the format. Never commits/pushes — you review the diff.
        await self._progress("project_pulse", 92, "Rendering managed docs")
        docs_written = 0
        all_stories = self.roadmap_store.list()
        for proj in projects:
            if not proj.get("manage_backlog"):
                continue
            repo = os.path.expanduser(proj.get("repo", ""))
            rel = proj.get("managed_file", "docs/BACKLOG.md")
            if not repo or not os.path.isdir(repo):
                continue
            try:
                doc = _render_backlog_doc(proj.get("name", ""), all_stories)
                fp = os.path.join(repo, rel)
        ...
        if docs_written:
            summary += f" · {docs_written} doc(s) managed"
        if top:
