...
def test_history_reports_measured_tokens_and_keeps_unrecorded_distinct_from_zero(tmp_path):
    """Attempt history reads the same ledger columns as the Switchboard."""
    ledger = RunLedger(tmp_path / "runs.sqlite3")
    agent = {"id": "agent", "name": "Synthetic"}
    base = {"at": "2020-01-01T00:00:00Z", "assignment_id": "work", "trigger": "assignment"}
    ledger.record(
        agent,
        {**base, "id": "measured", "result": "Evidence", "input_tokens": 1234, "output_tokens": 56},
    )
    ledger.record(
        agent, {**base, "id": "zero", "result": "Evidence", "input_tokens": 0, "output_tokens": 0}
    )
    ledger.record(agent, {**base, "id": "older", "result": "Evidence"})
    rows = {row["id"]: row for row in ledger.assignment_history("work")["runs"]}
    assert (rows["measured"]["input_tokens"], rows["measured"]["output_tokens"]) == (1234, 56)
    assert (rows["zero"]["input_tokens"], rows["zero"]["output_tokens"]) == (0, 0)
    assert rows["older"]["input_tokens"] is None
    assert rows["older"]["output_tokens"] is None
